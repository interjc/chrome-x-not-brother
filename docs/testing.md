...
15. 核对 X 页面 dock 的状态和四项概览，以及拦截规则行：未打开总开关时应显示未应用和规则条数；打开后显示生效中、有效规则数、本页拦截和累计拦截。点击“编辑规则”应打开侧栏规则页。点击本地化详情按钮确认打开当前标签页 Side Panel。悬停一名作者，点浮窗里紧挨名字的「不是兄弟！」后该 handle 应出现在规则并立即保存；打开帖子右上角三个点，菜单最上方同一项应写入该帖作者。在帖子正文划词后点「拦截关键词」应写入 contains 规则。这些入口都不得点击 X 的拉黑/静音按钮。
