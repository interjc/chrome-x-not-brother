...
- The script reads `following`, `followed_by`, `blocked_by`, `muting`, and already-present `name` / `profile_image_url_https` from the page UI store, tweet fibers (including ancestor components), and GraphQL responses the page itself already completed, to fill Home and thread cards that have no follow control and to repair display names and avatars the DOM extracted poorly. It does not start new GraphQL or REST requests.
...
- Main-world `page-bridge.js` only returns those already-loaded fields to the isolated-world observer. DOM evidence wins; store / completed responses only fill internal unknown.
...
- Send signatures are deduplicated only after confirmed persistence. A failed message or a service worker that did not return the matching users does not commit the signature, so a later rescan can retry.
