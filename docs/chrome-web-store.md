...
另外分别使用英语、日语和简体中文 Chrome UI 打开 Side Panel 与档案库，并把 X 设为不同语言验证注入 UI 的独立自动切换。再在 Side Panel 语言选择器中改成另一种支持语言，确认扩展页跟随选择、X 注入 UI 不跟着变。未支持的 Chrome/X 语言应完整回退英语，不能出现空白 `__MSG_*__` 或混合半套翻译。
