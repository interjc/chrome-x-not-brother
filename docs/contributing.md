...
不要提交会自动滚动 X、打开或预取资料页、调用 X 私有接口，或点击关注、取关、拉黑、静音及其他账号操作控件的代码。标准 X 资料链接只能由用户明确点击或键盘激活后打开。
...
用户可见字符串必须走 `src/i18n/`，并保持英语、日语、简体中文完整。扩展页跟随 Chrome UI 语言，注入 X 的界面跟随页面语言。用 DOM API 和文本内容注入 UI，不要把 HTML 字符串插入 X。运行时代码必须全部打进本地包，不能远程加载可执行代码。
