...
徽标插在 X 用户名区域，保持不可点击，避免与 X 原生控制混淆。时间线与评论区紧挨可见 `@handle` 之前；没有可见 handle 时才跟在显示名称后面。关注列表、相关用户和作者浮窗等 `UserCell` / `HoverCard` 把关系标签放在头像正下方，不插入显示名称行，也不覆盖右侧关注按钮。
...
| Unfollowed you | 对方取关 | 琥珀色 | 历史比对：互关或对方关注我变为我仍在关注的单向关注 |
| You unfollowed | 你已取关 | 蓝色 | 互关变为对方关注我 |
| Blocked you | 对方拉黑 | 红色 | 已知正常关系变为 blocked-by |
| Changed | 关系变化 | 玫红色 | 双方同时变化、无法单方归因的其他已知关系变化 |

关系变化是由 `previousRelationship` 与当前基础关系动态推导的覆盖显示状态，基础当前关系仍保存在记录中。当前已是互关时一律显示“互关”。当前是我单向关注时默认显示“单向关注”；只有历史比对证明对方曾经关注我、现在我仍关注且对方不再关注，才覆盖为“对方取关”。可以明确归因的单方取关或拉黑用四字短标：对方取关、你已取关、对方拉黑。`follows_you_only → none` 与其他双方都已取消关注一样不插入徽标，也不进入概览计数。只有双方同时变化的歧义切换保留通用“关系变化”，且不得把用户自己的关注/取关说成对方行为。用户在管理页确认后，徽标恢复显示当前基础关系。

`unknown` 只用于 adapter 表示证据不足，不是用户可见关系：不插入徽标、不写本地档案，旧版本的 unknown 记录也会被清理。重点关系采用更强的 ID 区域提示：我单向关注使用琥珀色左边线与底色，blocked-by 使用红色左边线、底色和带 `!` 的高对比矩形徽标。时间线与评论区的徽标仍插在 `@handle` 之前，X 的 handle 与日期元数据留在原生位置。帖子详情等不含时间戳的用户名区域：X 的显示名、认证和 `@handle` 收成一行，关系标签单独放在下一行；已含时间的祖先不改排版。左边线与显示名之间只留 1–2px。
...
Chrome 扩展菜单提供 Options/选项，打开与 Side Panel、档案库共享设置的选项页。时间线过滤默认关闭：打开后用 `display: none` 隐藏首页、搜索和通知中对应帖子单元格（含上方孤立的 “reposted” 社交上下文），不修改 X 账户状态。个人主页、帖子详情、作者浮窗和关注列表保持可见，避免用户无法取消静音或核对证据。
