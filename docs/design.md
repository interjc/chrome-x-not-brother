...
content script 在页面右下角插入一个不可与 X 原生控件混淆的固定 div：显示观察中/已暂停/尚未开启、已观察总数、单向关注、拉黑你和关系变化四项概览。“查看详情/打开侧栏”只打开扩展侧栏；未同意时主按钮明确写成“查看说明并同意”。右上角 `×` 将面板收为 54px NB 悬浮球，球上的状态点继续表达运行状态，点击球恢复完整概览；未同意时悬浮球旁另有琥珀色、带文字的同意入口，点击仍先打开完整披露，不直接同意。收起后的悬浮球比完整面板更靠页面内侧，给 X 右下角的 Grok/聊天按钮留出位置。收起偏好保存在 `chrome.storage.sync`，刷新、跨 X 标签页和 Chrome Sync 配置保持一致；未登录或关闭同步时仍保存在本机。这些交互都只改变扩展 UI，不触发 X 操作。窄屏时完整 dock 横向贴合安全边距；悬浮球与同意入口仍靠右下，同样向内侧让出原生聊天按钮。
