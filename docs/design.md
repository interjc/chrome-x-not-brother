...
content script 在页面右下角插入一个不可与 X 原生控件混淆的固定 div：显示观察中/已暂停/尚未开启、已观察总数、单向关注、拉黑你和关系变化四项概览。“查看详情/打开侧栏”只打开扩展侧栏；未同意时主按钮明确写成“查看说明并同意”。右上角 `×` 将面板收为 54px NB 悬浮球，球上的状态点继续表达运行状态，点击球恢复完整概览；未同意时悬浮球旁另有琥珀色、带文字的同意入口，点击仍先打开完整披露，不直接同意。收起偏好保存在 `chrome.storage.sync`，刷新、跨 X 标签页和 Chrome Sync 配置保持一致；未登录或关闭同步时仍保存在本机。这些交互都只改变扩展 UI，不触发 X 操作。窄屏时完整 dock 横向贴合安全边距，悬浮球与同意入口仍固定在右下安全边距。
...
完整编辑器放在宽屏档案库的可折叠“黑名单规则”区。区内先提供可扫描的四步上手流程、六类规则说明和可横向滚动/键盘聚焦的完整 JSON 示例，再进入表单。编辑器使用原生 label、fieldset、input、select、textarea 和按钮，保存/导入结果放在 `aria-live="polite"` 状态区；支持键盘焦点、窄屏单列、深浅主题和 reduced motion。每条规则卡明确显示类型、名称、启用、可选到期时间与用于导入合并的稳定 id。导入区把本地文件、公开 URL/Gist、合并/替换和下载分开呈现，并在 URL 下明确说明一次性访问与 Chrome 站点授权。
