...
`unknown` 只用于 adapter 表示证据不足，不是用户可见关系：不插入徽标、不写本地档案，旧版本的 unknown 记录也会被清理。重点关系采用更强的 ID 区域提示：我单向关注使用琥珀色左边线与底色，blocked-by 使用红色左边线、底色和带 `!` 的高对比矩形徽标。徽标插在 `@handle` 之前，X 的 handle 与日期元数据仍保留在原生位置；底色和边线不增加用户名区域的几何尺寸，避免评论卡片被撑成额外一行。
