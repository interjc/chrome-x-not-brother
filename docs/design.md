...
| Followed back | 回关了你 | 绿色 | 我单向关注变为互关 |
| Unfollowed you | 取关了你 | 琥珀色 | 互关变为我单向关注 |
| Blocked you | 拉黑了你 | 红色 | 已知正常关系变为 blocked-by |
| Changed | 关系变化 | 玫红色 | 发生了无法可靠归因于对方的其他已知关系变化 |

关系变化是由 `previousRelationship` 与当前基础关系动态推导的覆盖显示状态，基础当前关系仍保存在记录中。可以明确归因时显示“回关了你”“取关了你”或“拉黑了你”；涉及用户自己的关注变化等歧义切换保留通用“关系变化”。用户在管理页确认后，徽标恢复显示当前基础关系。
