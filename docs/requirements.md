...
20. 观察器运行但本地记录为零时，X 页面 dock 与 Side Panel 空状态必须明确提示：评论区需要用户悬停作者头像或 ID，由 X 显示关系浮窗后才能被动读取；不得让用户误以为观察器已扫描所有评论但没有结果。
...
22. 未确认的关系变化必须尽量显示可归因的具体事件：`following_only → mutual` 显示“回关了你”，`mutual → following_only` 显示“取关了你”，任意已知非 blocked 状态变为 `blocked_by` 显示“拉黑了你”。无法可靠归因于对方的其他变化保留通用“关系变化”，不得把用户自己的关注/取关误说成对方行为。X 页面观察 dock 只用一个“有变化”数字汇总所有未确认事件；Side Panel、页面徽标和档案记录显示具体事件。
