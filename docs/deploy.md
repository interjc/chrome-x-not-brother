...
- 上传包用 `artifacts/not-brother-<version>.zip`，不要手压 `dist/`。
...
- 扩展包构建与校验：`npm run release` 或 `npm run package`，上传包在 `artifacts/not-brother-<version>.zip`
...
按 [发布指南](release.md) 用 `npm run version:bump` 和 `npm run release` 升版本、打包。Dashboard 上传 `artifacts/` 里的新 ZIP，不是增量。权限或收集范围变了，先改 `terms/`、`pages/` 和同意版本。出问题用 Dashboard 回滚，不要静默清空用户 IndexedDB。
