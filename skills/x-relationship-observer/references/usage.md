...
The side panel, fieldbook, and toolbar copy default to the Chrome UI language. The side panel and fieldbook language switcher can override that with English, Japanese, or Simplified Chinese; the choice is stored in extension settings as `uiLocale` and defaults to follow-the-browser. A manual choice also updates relationship badges and the observer dock on x.com; `auto` keeps those surfaces on the X page language. Unsupported locales fall back to English.
