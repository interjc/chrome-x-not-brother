...
23. Keep the Side Panel on Status versus Options tabs, persist `sidePanelTab`, and let Chrome's action Options menu open the Options tab without extra permissions. Keep open Side Panel, dashboard, and Options settings synchronized through the shared `chrome.storage.onChanged` hook so viewer exclusion, observer state, optional timeline filters, the side-panel tab, and the UI language preference cannot go stale across extension contexts; remove the listener on unmount.
