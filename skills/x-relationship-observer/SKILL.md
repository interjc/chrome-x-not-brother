...
12. Preserve toolbar state, install guidance, the X-page observer dock's persistent panel/floating-ball behavior, and light/dark parity. Before current consent, keep an explicit disclosure entry in the expanded dock, beside the collapsed floating button, and in the action-icon context menu; these entries open the full notice and never consent automatically. Acid-yellow surfaces (active Status/Options tab, observer switch, primary buttons, active filters) keep dark ink `#16221b` in both themes so text does not invert onto the lime fill.
...
23. Keep the Side Panel on Status versus Options tabs, persist `sidePanelTab`, and let Chrome's built-in action Options menu open the Options tab through `options_ui`. The custom consent entry uses only `contextMenus`, is visible only before current consent, and opens Side Panel immediately from the click gesture with dashboard fallback. Store small preferences in `chrome.storage.sync`, keep `viewerHandle` and the full filter-rule document in `chrome.storage.local`, and never sync users/observations/rules. Keep open Side Panel, dashboard, and Options settings synchronized through `chrome.storage.onChanged` so viewer exclusion, observer state, optional timeline filters, filter rules, the side-panel tab, the consent menu, and the UI language preference cannot go stale across extension contexts; remove listeners on unmount.
24. Keep filter-rules v1 strict and local: Zod-validate handles/display names/content matchers, enabled state, and expiration; reject unsafe regex and over-limit documents. Remote JSON/Gist import is a one-time explicit user action behind optional per-source HTTPS host permission, with no credentials, subscriptions, remote code, or local/X data upload. Content text is matched in memory and never persisted.
